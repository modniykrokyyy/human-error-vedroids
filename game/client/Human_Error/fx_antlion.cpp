...
// Use all the gibs
#define	NUM_CRICKET_GIBS 4
const char *pszCricketGibs[NUM_CRICKET_GIBS] = {
	"models/gibs/crigib_head.mdl",
	"models/gibs/crigib_buttocks001.mdl",
	"models/gibs/crigib_middle001.mdl",
	"models/gibs/crigib_middle002.mdl",
};

void FX_CricketGib( const Vector &origin, const Vector &direction, float scale )
{
	Vector	offset;

	// Spawn all the unique gibs
	for ( int i = 0; i < NUM_CRICKET_GIBS; i++ )
	{
		offset = RandomVector( -16, 16 ) + origin;

		C_AntlionGib::CreateClientsideGib( pszCricketGibs[i], offset, ( direction + RandomVector( -0.8f, 0.8f ) ) * ( 150 * scale ), RandomAngularImpulse( -32, 32 ), 2.0f);
	}
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CricketGibCallback( const CEffectData &data )
{
	FX_CricketGib( data.m_vOrigin, data.m_vNormal, data.m_flScale );
}

DECLARE_CLIENT_EFFECT( "CricketGib", CricketGibCallback );
