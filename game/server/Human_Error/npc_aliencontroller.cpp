...
void CNPC_AlienController::PrescheduleThink( void )
{
	...
	//	!HasCondition( COND_ALIENCONTROLLER_CAN_PHYS_ATTACK ) &&
...
/*void CNPC_AlienController::ParticleMessages(int message)
{
	switch ( message )
	{
	case MESSAGE_START_FIREBALL:
		{
			unsigned char uchAttachment1 = m_iRightHandAttachment;
			unsigned char uchAttachment2 = m_iLeftHandAttachment;	
			EntityMessageBegin( this, true );
				WRITE_BYTE( 0 );
				WRITE_BYTE( uchAttachment1 );
				WRITE_BYTE( uchAttachment2 );
			MessageEnd();
		}
		break;
	case MESSAGE_STOP_FIREBALL:
		{
			EntityMessageBegin( this, true );
				WRITE_BYTE( 1 );
			MessageEnd();
...
void CNPC_AlienController::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_CONTROLLER_FLY )
	{
		...
		m_OnLiftOff.Set( 1.0f, this, this );
...
int CNPC_AlienController::SelectSchedule( void )
{
	...
	/*if ( m_bIsFlying && ( m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT || m_NPCState == NPC_STATE_IDLE ) )
	{
		...
		return SCHED_ALIENCONTROLLER_LEVITATE;
	}*/
...
void CNPC_AlienController::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask)
	{
	/*case TASK_STOP_MOVING:		//TERO: I commented this out because it doesn't take into count that we still use the OverrideMove
		{
			if (m_bIsFlying)
			{
...
#ifdef CONTROLLER_PATH_FINDING_DEBUG_MESSAGES
	if (GetCurSchedule())
		DevMsg("npc_aliencontroller: running schedule with name: %s, with task id %d\n", GetCurSchedule()->GetName(), pTask->iTask );
	else
		DevMsg("npc_aliencontroller: running schedule with task: %d\n", pTask->iTask );
#endif
	...
	switch ( pTask->iTask )
	{
		/*case TASK_WAIT_FOR_MOVEMENT:
		{
			if (m_bIsFlying)
			{
				...
				if (fTimeExpired || GetNavigator()->GetGoalType() == GOALTYPE_NONE || !GetNavigator()->IsGoalActive())
				{
					TaskComplete();
...
void CNPC_AlienController::Land(bool bIsFlying)
{
	...
	/*if (GetTarget())
		StartTargetHandling(GetTarget());

	if (vecGoalPos != vec3_origin)
	{
		//GetNavigator()->SetGoal( vecGoalPos );

		Vector vecGoal = vecGoalPos;
		AI_NavGoal_t goal(vecGoal, ACT_FLY, AIN_DEF_TOLERANCE, AIN_YAW_TO_DEST);
	...
	/*if (GetNavigator()->IsGoalActive())
		GetNavigator()->BuildAndGetPathDistToGoal();*/
	//GetNavigator()->ClearGoal(); //TERO: this seems to crash the game
...
void CNPC_AlienController::OnScheduleChange ( void )
{
	...
	//TERO: this is what is different
	//GetNavigator()->ClearGoal();
...
bool CNPC_AlienController::ControllerProgressFlyPath()
{
	...
	/*Vector vecDist = GetNavigator()->GetCurWaypointPos() - (GetAbsOrigin() + CONTROLLER_BODY_CENTER);
	float waypointDist  = VectorNormalize(vecDist);

	Vector vecTravel = (GetAbsOrigin() - m_vLastFlyPosition);
	float flTravel = VectorNormalize(vecTravel);

	float flDot = DotProduct(vecTravel, vecDist);

	float flSpeed = GetAbsVelocity().Length();

	NDebugOverlay::Line( m_vLastFlyPosition,					GetAbsOrigin(),	255, 0, 0, true, 0.1 );
	NDebugOverlay::Line( GetNavigator()->GetCurWaypointPos(),	GetAbsOrigin(),	0, 255, 0, true, 0.1 );

	if (flDot < 0.0f && flTravel <= 50.0f && waypointDist <= flTravel )
	{
		SetAbsOrigin(GetNavigator()->GetCurWaypointPos() - CONTROLLER_BODY_CENTER);
		m_vLastFlyPosition = GetAbsOrigin();
	...
	DevMsg("dist %f, travel dist %f, speed %f, dot %f\n", waypointDist, flTravel, flSpeed, flDot);*/
...
#ifdef CONTROLLER_MOVE_DEBUG_MESSAGES
			DevMsg("npc_aliencontroller: simplifying fly path\n");
#endif
...
{	
	if (!IsInAScript())
	{
		...
		if (m_bIsLanding)
			DevMsg("IS LANDING!!!\n");
		if (HasCondition(COND_ALIENCONTROLLER_SHOULD_LAND))
			DevMsg("HAS CONDITION SHOULD LAND!");
		DevMsg("NEXT LANDING CHECK %f\n", m_flNextLandingTime - gpGlobals->curtime );
		DevMsg("NUMBER OF PHYSICS ENTS %d\n", m_iNumberOfPhysiscsEnts);*/
	...
	if (HasCondition( COND_ALIENCONTROLLER_FLY_BLOCKED) ) // &&  !GetHintNode() ) //( m_flNextHintTime < gpGlobals->curtime ||
	{
		//DevMsg("npc_aliencontroller: Condition fly blocked\n");
		...
		if (GetNavigator()->IsGoalActive()) // && m_vOldGoal == vec3_origin)
		{
			...
			bGotPath = GetNavigator()->PrependLocalAvoidance( controller_avoid_dist.GetFloat(), moveTrace );
			ClearCondition( COND_ALIENCONTROLLER_FLY_BLOCKED );*/
...
void CNPC_AlienController::BuildScheduleTestBits( void )
{
	if ( !HasCondition( COND_ALIENCONTROLLER_SHOULD_LAND ) && HasCondition( COND_SEE_ENEMY) &&
		 ...
		 !IsCurSchedule(SCHED_ALIENCONTROLLER_FIREBALL_EXTINGUISH) &&
		 m_iNumberOfPhysiscsEnts > 0 )
	{
...
/*bool CNPC_AlienController::LineToPhysicsEntIsClear( void )
{
	if (m_hPhysicsEnt)
	{
		trace_t tr;
		AI_TraceHull( GetAbsOrigin(), m_hPhysicsEnt->WorldSpaceCenter(), ALIENCONTROLLER_HULL_MINS, ALIENCONTROLLER_HULL_MAXS, MASK_NPCSOLID, this, COLLISION_GROUP_NPC, &tr );
	
		if (tr.fraction == 1 || tr.m_pEnt == m_hPhysicsEnt)
			return true;
	}
...
bool CNPC_AlienController::FindNearestPhysicsObjects( int iMaxMass )
{
	...
	for (i=0; i<ALIENCONTROLLER_NUMBER_OF_TELEKINESIS_OBJECTS; i++)
	{
		...
		flNearestDist[i] = min( dist * 2.0f, (float)ALIENCONTROLLER_FARTHEST_PHYSICS_OBJECT );
	}
...
void CNPC_AlienController::NPCThink( void )
{
	...
	if (IsCurSchedule( SCHED_ALIENCONTROLLER_TELEKINESIS ) && m_iNumberOfPhysiscsEnts > 0)
	{
		...
		/*DevMsg("npc_aliencontroller: time to throw next object: %f\n", m_flNextTelekinesisThrow - gpGlobals->curtime);
		DevMsg("npc_aliencontroller: time to cancel next object: %f\n", m_flNextTelekinesisCancel - gpGlobals->curtime);
		...
		if (m_flNextTelekinesisThrow < gpGlobals->curtime)
		{
			DevMsg("TIME: OK, ");
		}
		else
...
void CNPC_AlienController::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
