...
#ifdef FIREBALL_USE_SPRITE_FIREGLOW
	DEFINE_FIELD( m_pFireGlow,				FIELD_CLASSPTR ),
#endif
...
//-----------------------------------------------------------------------------
/*bool CFireBall::CreateVPhysics( void )
{
	// Create the object in the physics system
	VPhysicsInitNormal( SOLID_BBOX, FSOLID_NOT_SOLID | FSOLID_TRIGGER, false );

	return true;
}*/

#ifdef FIREBALL_USE_SPRITE_FIREGLOW
void CFireBall::CreateFireGlow()
{
	//m_pFireGlow = CBeam::BeamCreate( CONTROLLER_FIRE_SPRITE_TAIL, 64 );
...
//		m_pFireGlow->SetAttachment( this, m_iFireBallAttachment );
		m_pFireGlow->SetParent(this);
		m_pFireGlow->SetBrightness( 255 );
		m_pFireGlow->SetScale( 0.4 );

		/*m_pFireGlow->SetColor( 255, 255, 255 ); 

		Vector forward = GetAbsVelocity();
		//GetVectors( &forward, NULL, NULL );
		VectorNormalize(forward);

		m_pFireGlow->RelinkBeam(); 

		m_pFireGlow->PointsInit( GetLocalOrigin() - (forward), GetLocalOrigin()  );

		m_pFireGlow->SetParent( this );

		m_pFireGlow->SetBrightness( 255 );

		m_pFireGlow->SetFadeLength( 0 );

		m_pFireGlow->SetHaloTexture(m_nHaloSprite);
		m_pFireGlow->SetHaloScale(32);

		m_pFireGlow->SetEndWidth(64);

		m_pFireGlow->SetNoise( 0 );*/

		//m_pFireGlow->SetBeamFlags( (FBEAM_SHADEOUT|FBEAM_NOTILE) );
		//m_pFireGlow->SetBeamFlags( FBEAM_NOTILE );

		//m_pFireGlow->SetStartPos( GetLocalOrigin() );
		//m_pFireGlow->SetEndPos( GetLocalOrigin() - (64 * forward) );
	}
}
#endif
...
void CFireBall::Explode( void )
{
	...
	if( tr.fraction > 0.5 || !(tr.surface.flags & SURF_SKY) )
	{
		if ( tr.m_pEnt && tr.m_pEnt->IsNPC() )
			bMadeFire = FireSystem_StartFire( (CBaseAnimating *) tr.m_pEnt->MyNPCPointer(), 64, 4, 6, SF_FIRE_START_ON | SF_FIRE_START_FULL | SF_FIRE_SMOKELESS, NULL, FIRE_NATURAL);
		else
