...
int	CNPC_Stalker::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	// --------------------------------------------
	//	Don't take a lot of damage from Vortigaunt... or Alien Grunts... or Alien Controllers, lets not forget about Controller!
	//  And if we don't want to take damage from Grunts, then we don't want to take damage from their bees either!
	//  And the fireballs of the Controllers are defined as CLASS_BEE, so, that's double no for bees. And of course it would
	//  be super good if rebels didn't give any damage either, so... lets not take much damage from them either, okay? OKAY?
	// --------------------------------------------
	if (info.GetAttacker()->Classify() == CLASS_VORTIGAUNT || 
		...
		info.GetAttacker()->Classify() == CLASS_CITIZEN_REBEL )
	{
